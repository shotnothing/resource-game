...
export default function ReserveButton({ card, doAction }: { card: GameCard, doAction: DoActionType }) {
  ...
  export function ReserveTierButton({ tier, doAction }: { tier: string, doAction: DoActionType }) {
    const [dialogOpen, setDialogOpen] = useState(false);
    ...
    return (
        ...
        <DialogContent className="sm:max-w-[450px]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold">Reserve a Random Tier {tier} Card</DialogTitle>
          </DialogHeader>
